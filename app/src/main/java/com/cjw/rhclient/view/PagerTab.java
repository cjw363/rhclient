...
public class PagerTab extends ViewGroup {
	...
	private int mIndicatorColor = R.color.colorPrimaryDark;//指示器颜色
	...
	private int mTabBackgroundResId = R.color.transparent;// tab背景资源
	private int mTabTextColorResId = R.color.black; //tab文字颜色
	...
	private void init() {
		//把一个值从dip转换成px
		mIndicatorHeight = UI.dip2px(mIndicatorHeight);
		mDividerPadding = UI.dip2px(mDividerPadding);
		mTabPadding = UI.dip2px(mTabPadding);
		mDividerWidth = UI.dip2px(mDividerWidth);
		mTabTextSize = UI.dip2px(mTabTextSize);
		//创建一个scroller
	...
	private void initPaint() {
		...
		mIndicatorPaint.setColor(UI.getColor(mIndicatorColor));
	...
	private void addTextTab(final int position, String title) {
		...
		tab.setTypeface(Typeface.defaultFromStyle(Typeface.NORMAL));//设置字体粗细
		tab.setTextColor(UI.getColorStateList(mTabTextColorResId));
		tab.setBackgroundDrawable(UI.getDrawable(mTabBackgroundResId));
